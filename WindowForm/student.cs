...
namespace WindowForm
{
    public partial class student : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            con.Open();
            SqlCommand cmd = new SqlCommand("insert into studentdb values (@id,@name,@roll,@email,@phone)",con);
            cmd.Parameters.AddWithValue("@Id", textid.Text);
            cmd.Parameters.AddWithValue("@Name", textname.Text);
            cmd.Parameters.AddWithValue("@Roll", textroll.Text);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            SqlConnection con = new SqlConnection("@Data Source=DESKTOP-CC5PJC1\\SQLEXPRESS;Initial Catalog=ConnectionDb;Integrated Security=True;Trust Server Certificate=True");
            con.Open();
            SqlCommand cmd = new SqlCommand("Update studentdb Set name=@name,roll=@roll,email=@email,phone=@phone Where id = '1' ",con);
            cmd.Parameters.AddWithValue("@Id", int.Parse(textid.Text));
            cmd.Parameters.AddWithValue("@Name", textname.Text);
            cmd.Parameters.AddWithValue("@Roll", int.Parse(textroll.Text));
            cmd.Parameters.AddWithValue("@Email", textemail.Text);
            cmd.Parameters.AddWithValue("@Phone", textphone.Text);
            cmd.ExecuteNonQuery();
            con.Close();
